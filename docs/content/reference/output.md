...
describes what users get. Pick a format with `-o`, or let gaia choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gaia <command> -o table   # aligned columns for reading
gaia <command> -o jsonl   # one JSON object per line, for piping
gaia <command> -o json    # a single JSON array
gaia <command> -o csv     # spreadsheet friendly
gaia <command> -o tsv     # tab-separated
gaia <command> -o url     # just the URL column
gaia <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gaia <command> --fields id,title,url
```
...
```bash
gaia <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gaia <command>            # a table, because this is a terminal
gaia <command> | wc -l    # JSONL, because this is a pipe
```
